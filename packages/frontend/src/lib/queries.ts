...
export function useHealth() {
  return useQuery({ queryKey: queryKeys.health, queryFn: api.getHealth, refetchInterval: 5000 });
}
export function useCharacters(params?: { active_only?: boolean }) {
  return useQuery({ queryKey: queryKeys.characters(params), queryFn: () => api.getCharacters(params) });
}
export function useCharacter(id: string) {
  return useQuery({ queryKey: queryKeys.character(id), queryFn: () => api.getCharacter(id), enabled: !!id });
}
export function useWorld() {
  return useQuery({ queryKey: queryKeys.world, queryFn: api.getWorld, refetchInterval: 5000 });
}
...
export function useMemories(characterId: string, limit = 20) {
  return useQuery({ queryKey: queryKeys.memories(characterId), queryFn: () => api.getMemories(characterId, limit), enabled: !!characterId });
}
export function useMessages(characterId: string, limit = 20) {
  return useQuery({ queryKey: queryKeys.messages(characterId), queryFn: () => api.getHistory(characterId, limit), enabled: !!characterId });
}
export function useScenes() {
  return useQuery({ queryKey: queryKeys.scenes, queryFn: api.getScenes });
}
export function useAdminStatus() {
  return useQuery({ queryKey: queryKeys.adminStatus, queryFn: api.getAdminStatus, refetchInterval: 10000 });
}

export function useSendMessage() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ characterId, userId, content }: { characterId: string; userId: string; content: string }) =>
