...
def fake_db(monkeypatch):
    ...
    session = FakeSession()
    monkeypatch.setattr(messages_module.db, "session", lambda: FakeSessionCtx(session))
...
async def test_list_conversations_rejects_other_user(fake_db):
...
async def test_list_conversations_exact_query_uses_authed_user(fake_db):
    await list_conversations(
        current_user={"user_id": "user-a"},
...
async def test_list_conversations_character_branch_filters_user(fake_db):
    await list_conversations(
        current_user={"user_id": "user-a"},
        character_id=str(_CHARACTER_ID),
    )
...
async def test_list_conversations_no_filter_returns_own_only(fake_db):
