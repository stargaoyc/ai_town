...
| 维度 | yuiju | aitown |
|------|-------|--------|
| 提示词存放位置 | TypeScript 源码内嵌（`packages/utils/src/prompt/*.ts`） | YAML 外置（`configs/prompts/*.yaml`） |
| 角色身份 | 硬编码单角色 ゆいじゅ/悠酱 | 多角色，`{name}`/`{personality}` 模板变量 |
| 人格分层 | 分 5 层：客观设定 / 聊天人格 / 状态表达 / 金币隐私 / 决策版人设 | 单层 `[角色档案]` 块，仅 name/personality/backstory |
| 说话风格约束 | 极细腻：说话温度、卡顿规则、颜文字原则、寂寞底色、轻文学感、被温柔碰到的反应 | 仅"符合角色性格"、"简洁有趣"、"不暴露 AI" |
| 场景化人设 | 聊天与决策使用不同人设片段（`characterPersonalityPrompt` vs `characterDecisionPrompt`） | 聊天与决策 Prompt 共用同一份角色档案 |
| 状态泄露防护 | 显式约束：数值不外泄（`characterStateExpressionPrompt`）、金币模糊表达（`characterMoneyPrivacyPrompt`） | 无独立的状态泄露防护约束 |
| 跨世界边界 | 显式声明：用户与角色不在同物理世界，禁止编造共同物理行动 | 无此约束 |
| 工具使用规则 | 内嵌于角色卡：回忆走 `todayEventSearch`/`diarySearch`，工具返回为客观事实 | 未在 Prompt 中声明工具使用规则 |
| 关系与边界 | 详尽：普通请求 vs 冒犯的区分、拒绝方式、被温柔碰到时的反应模式 | 未涉及 |
| 真实感原则 | 显式三条：真实比圆滑重要 / 克制比表演重要 / 主体性不能丢 | 仅"人性化，避免暴露 AI 模型" |

### aitown 可借鉴的改进点

1. **人设分层**：将 `chat.yaml` 中的 `[角色档案]` 拆分为「客观设定」与「聊天人格」两层，客观设定供聊天/决策/反思共享，聊天人格仅用于消息回复。避免决策 Prompt 被聊天语气污染。
2. **状态泄露防护**：在 `chat.yaml` 增加 `[状态表达规则]` 段，约束角色不得向用户播报 stamina/satiety/mood 的具体数值，仅用自然口语表达感受。
3. **场景化人设片段**：新增 `decision-personality.yaml` 片段，描述「角色如何取舍、如何生活」，与聊天人格解耦。yuiju 的 `characterDecisionPrompt`（"状态先于计划"、"不追求数值最优"）是很好的范本。
4. **跨世界边界声明**：在 `chat.yaml` 增加 `[世界边界]` 段，明确角色不得声称与用户共同进行物理行动，所有交互通过消息进行。这对多角色小镇尤为重要。
5. **工具使用规则内嵌**：当 aitown 引入 MCP 工具检索记忆时，应在 Prompt 中显式声明「回忆走检索工具，工具返回为客观事实，与用户描述冲突时以工具为准」。
6. **真实感三原则**：在 `[严格约束]` 中加入 yuiju 的三条原则（真实 > 圆滑、克制 > 表演、保留主体性），显著降低模型输出模板腔概率。
...
| 改进点 | 落地位置 | 预期收益 |
|--------|----------|----------|
| 人设分层（客观设定 vs 聊天人格 vs 决策人设） | `configs/prompts/` 拆分片段 | 决策 Prompt 不被聊天语气污染，决策质量提升 |
| 状态泄露防护约束 | `chat.yaml` 增加 `[状态表达规则]` | 角色不再向用户播报数值，沉浸感提升 |
| 真实感三原则 | `chat.yaml` `[严格约束]` 段 | 降低模板腔概率 |
| LLM 模型分档 | `config.py` + `LLMClient` | 成本显著下降 |

### P1：短期借鉴

| 改进点 | 落地位置 | 预期收益 |
|--------|----------|----------|
| 跨世界边界声明 | `chat.yaml` 增加 `[世界边界]` 段 | 避免角色编造与用户的物理共同行动 |
| 计划变更审查机制 | `plan_repo.py` + 决策 Prompt | 减少 LLM 频繁重写计划 |
| 记忆热度机制 | `relations` 表增加 `interaction_count` | 人物记忆检索更精准 |
| 每日行为去重 | `character_states` 增加 `daily_actions_done` 字段 | 防止重复执行每日一次类行为 |

### P2：中期借鉴

| 改进点 | 落地位置 | 预期收益 |
|--------|----------|----------|
| 静态知识工具化 | MCP 工具 `query_world_guide` | 决策 Prompt 不再膨胀 |
| 场景化人设片段 | 新增 `decision-personality.yaml` | 决策更贴合角色个性 |
| 表情包配置化 | `configs/stickers.yaml` | 表情包可配置，角色卡引用稳定 key |
| 命令队列模式 | `core/world_engine.py` | 跨角色资源竞争原子化 |
