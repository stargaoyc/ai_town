...
> P1-12 漂移修正：当前**不存在独立的定时规划生成器**。计划完全由 LLM 在
> Character Tick 决策中通过 `createPlanChanges` 自发创建（被动涌现），
> Action 执行后另有启发式计划-行动对账自动推进进度（`plan_auto_progress_*` 配置）。
