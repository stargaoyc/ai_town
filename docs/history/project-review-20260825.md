...
## 附记：本报告问题清单的修复状态（2026-08-25 修复批）

全部 HIGH 与 MEDIUM 问题已在本轮修复批内关闭，按提交分组：

| 提交 | 覆盖问题 | 状态 |
|---|---|---|
| `9c9354b` fix(backend) | H1 日记世界时钟对齐、H2 幽灵反思计数、H9 分区保留生命周期、M1-M7、M24、L1、L7 | ✅ 已修 |
| `58807c5` fix(messaging) | H3 XACK 语义+流裁剪、H4 去重时序、H5 自消息排除+问候闸门 | ✅ 已修 |
| `dfd293b` fix(deploy) | H6 凭据/端口、H7 环境变量补全、H8 抓取目标、L11-L16 文档校准 | ✅ 已修 |
| `f7b2419` fix(frontend) | M19-M23、L8-L10（含 toast/确认体系、移动导航、a11y 快赢） | ✅ 已修 |
| `517e9d4` fix(tick) | H10 失锁中止、M8-M11 并发收尾 | ✅ 已修 |
| `c7d738f` fix(messaging) | M12-M18 消息域长尾（含 M15 判定档位接线） | ✅ 已修 |

### 有意保留 / 裁决说明

| 项 | 处置 | 理由 |
|---|---|---|
| L2 chat 面不注入反思/日记 | 维持现状并文档化 | 对话上下文聚焦用户关系（Person Memory 已注入）；反思/日记属镇内决策语义，避免对话 Prompt 过载。若未来需要，chat.yaml 增占位符即可 |
| L4 通知列表无上限 | **误报关闭** | 复核发现 `runtime.py:272-273` 写入侧已有 `ltrim` 保留 200 条上限，数据层审查该项结论不成立 |
| L6 pg_uuidv7 扩展未在迁移中创建 | 以文档裁决 | PG18 内建 `uuidv7()`，官方 pgvector:pg18 镜像开箱可用；README 已声明 PostgreSQL 18+ 为硬性要求，非 Docker 环境同样满足 |
| 战略项：agent-agent 多轮对话与关系质量化 | 规划中 | 见 §六战略级清单，超出缺陷修复范围 |
| 战略项：QQ 免打扰时段与按用户节流 | 规划中 | 同上 |

### 修复批验证基线

后端：ruff check/format 全过、mypy --strict **202 文件零错误**、pytest **516 passed + 13 skipped**（含分区保留生命周期与队列语义的真实 PG18 集成测试）；
前端：lint/typecheck/vitest 全过（**52 tests**）；`docker compose config` 双向验证通过（有凭据 exit 0 / 无凭据 exit 1 证明 `:?` 门禁生效）。
