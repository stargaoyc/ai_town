...
本项目构建一个由 LLM 驱动的**多智能体虚拟小镇**，让二次元 AI 角色拥有独立的记忆、反思、规划与社交能力，在持续运行的虚拟世界中自主生活，并通过 Web、QQ（OneBot）等多渠道与用户建立长期陪伴关系。（注：飞书/Lark 渠道为规划中未落地，`platform` 枚举预留 `lark` 值，无实际适配器。）
